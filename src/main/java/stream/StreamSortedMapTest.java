...
public class StreamSortedMapTest {
    public static void main(String[] args){
        ...
        try {
            ...
            employeeMap.entrySet().stream().sorted((e1,e2) ->e1.getValue().getSalary() - e2.getValue().getSalary()).forEach(System.out::println);
            System.out.println("-------------------------------------------------------------------------");
            employeeMap.entrySet().stream().sorted(Comparator.comparing(e -> e.getValue().getName())).forEach(System.out::println);
            System.out.println("------------------------------Reversed Order-------------------------------------------");
           // Stream<Map.Entry<Integer, Employee>> empset = employeeMap.entrySet().stream();
            employeeMap.entrySet().stream().sorted((e1,e2) -> -e1.getValue().getName().compareTo(e2.getValue().getName())).forEach(System.out::println);
            System.out.println("------------------------------Reversed Order-------------------------------------------");
